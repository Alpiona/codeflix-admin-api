  ...
  describe('DB Schema', () => {
    ...
    describe('DB_LOGGING', () => {
      test('invalid cases', () => {
        ...
        expectValidate(schema, { DB_LOGGING: 4 }).toContain(
          '"DB_LOGGING" must be a boolean',
        );
      });

      test('valid cases', () => {
        const arrange = [false, true, 'false', true];
    ...
    describe('DB_AUTO_LOAD_MODELS', () => {
      test('invalid cases', () => {
        ...
        expectValidate(schema, { DB_AUTO_LOAD_MODELS: 4 }).toContain(
          '"DB_AUTO_LOAD_MODELS" must be a boolean',
        );
      });
